    ...
    <div className="flex flex-col gap-6 pb-8 h-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Gestiona las áreas geográficas y mercados operativos.</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-xl border border-brand-gray-border shadow-sm flex flex-col flex-1 min-h-0">
        <div className="p-4 border-b border-brand-gray-border flex bg-slate-50/50 rounded-t-xl">
        ...
        <div className="overflow-x-auto flex-1 p-4">
          {isLoading ? (
            <div className="text-center text-slate-500 py-10">Cargando mercados...</div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 text-xs font-semibold text-slate-500 uppercase border-b border-slate-200">
                  ...
                  <th className="px-4 py-3">Ciudad (ID)</th>
              ...
              <tbody className="text-sm">
                {filteredData.map(m => (
                  <tr key={m.id_mercado} className="border-b border-slate-100 hover:bg-slate-50">
                    <td className="px-4 py-4 text-slate-500 font-mono">{m.id_mercado}</td>
                    <td className="px-4 py-4 font-bold text-slate-800">{m.nombre}</td>
                    <td className="px-4 py-4 text-slate-600">{m.id_ciudad}</td>
                    <td className="px-4 py-4 text-slate-600 truncate max-w-[200px]">{m.direccion || 'Sin dirección'}</td>
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-slate-600 mb-1">Nombre del Mercado</label>
              <input type="text" required value={formData.nombre} onChange={e => setFormData({...formData, nombre: e.target.value})} placeholder="Ej. Mercado Rodriguez" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm" />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <input type="text" value={formData.direccion} onChange={e => setFormData({...formData, direccion: e.target.value})} placeholder="Dirección física" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm" />
            </div>

            <div className="md:col-span-2 border-t pt-4 mt-2">
              <label className="block text-sm font-bold text-slate-800 mb-2">Ubicación en el Mapa</label>
              <div className="flex gap-2 mb-4">
                <div className="w-1/2">
                  <label className="block text-xs font-medium text-slate-500 mb-1">Latitud</label>
                  <input type="number" step="any" value={formData.latitud} onChange={e => setFormData({...formData, latitud: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50" />
                </div>
                <div className="w-1/2">
                  <label className="block text-xs font-medium text-slate-500 mb-1">Longitud</label>
                  <input type="number" step="any" value={formData.longitud} onChange={e => setFormData({...formData, longitud: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50" />
                </div>
              </div>

              <div className="h-40 w-full bg-slate-100 rounded-lg overflow-hidden border border-slate-300 relative">
                <p className="absolute top-2 left-2 z-[400] bg-white/90 px-2 py-1 text-[10px] font-bold rounded shadow pointer-events-none">Haz clic para fijar la coordenada del mercado</p>
          ...
          <div className="mt-4 pt-4 border-t flex justify-end gap-3">
            ...
            <button type="submit" className="px-4 py-2 bg-brand-blue text-white rounded-lg text-sm font-bold flex items-center gap-2"><Save size={16}/> Guardar</button>
          </div>
        </form>
      </Modal>
    </div>
