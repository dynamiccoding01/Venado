    ...
    <div className="flex flex-col gap-6 pb-8 h-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Gestiona los tipos de clientes y sus perfiles de atención.</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-xl border border-brand-gray-border shadow-sm flex flex-col flex-1 min-h-0">
        ...
        <div className="overflow-x-auto flex-1 p-4">
          {isLoading ? (
            <div className="text-center text-slate-500 py-10">Cargando categorías...</div>
          ) : (
              ...
              <tbody className="text-sm">
                {filteredData.map(c => (
                  <tr key={c.id_categoria} className="border-b border-slate-100 hover:bg-slate-50">
                    <td className="px-4 py-4 text-slate-500 font-mono">{c.id_categoria}</td>
                    <td className="px-4 py-4 font-bold text-slate-800">{c.nombre}</td>
                    <td className="px-4 py-4 text-slate-600 max-w-[200px] truncate">{c.criterio_clasificacion}</td>
                    <td className="px-4 py-4 text-slate-600 max-w-[250px] text-xs leading-relaxed">{c.perfil_atencion || 'Sin descripción'}</td>
                    <td className="px-4 py-4 text-center font-bold text-brand-blue bg-blue-50/30">{c.tiempo_promedio_visita_min}'</td>
                    <td className="px-4 py-4 text-center">
                      <span className={clsx("px-2 py-1 rounded text-[10px] font-bold uppercase", c.activo ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700")}>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Nueva Categoría de Cliente">
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          <div>
            ...
            <input type="text" required value={formData.nombre} onChange={e => setFormData({...formData, nombre: e.target.value})} placeholder="Ej. MINORISTA" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm" />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-600 mb-1">Criterio de Clasificación</label>
            <input type="text" required value={formData.criterio_clasificacion} onChange={e => setFormData({...formData, criterio_clasificacion: e.target.value})} placeholder="Ej. Más de Bs. 5,000 de compra" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm" />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-600 mb-1">Tiempo Promedio de Visita (min)</label>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-600 mb-1">Perfil de Atención</label>
            <textarea rows="3" value={formData.perfil_atencion} onChange={e => setFormData({...formData, perfil_atencion: e.target.value})} placeholder="Descripción detallada sobre cómo se debe atender a esta categoría..." className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm resize-none"></textarea>
          </div>
